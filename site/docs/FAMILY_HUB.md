...
| Calendar (agenda + branded month grid) | Upcoming agenda (type-coloured) + `HubCalendarGrid` month view (desktop); clicking an agenda card or a grid day opens `HubEventDialog` (details + add-to-calendar) | `googleCalendarId` / feed in Site Settings         |
