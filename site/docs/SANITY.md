...
- **Self-service extras (board independence):** a `redirect` document type (Everything → Site setup → Redirects) whose entries are read at build by `astro.config.mjs`'s `fetchCmsRedirects()` and folded into the `redirects` map — so the board fixes old/renamed links without the developer (see [REDIRECTS.md](REDIRECTS.md)). An **Export** Studio tool (`src/sanity/components/ExportTool.tsx`, Everything workspace) downloads subscribers / form submissions / family directory as CSV client-side (no endpoint, uses the signed-in session) — the off-boarding path. `update.showUntil` lets a pinned announcement drop off the hub home on its own.
