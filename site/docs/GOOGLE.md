...
| **Calendar feed web app**      | Apps Script serving the calendar as JSON ([source](../scripts/apps-script/calendar-feed.gs))                                                        | Maintainer's personal account (redeployed 2026-07-17)             | Redeploy from the committed source under any account that can see the calendar; update Site Settings → Calendar feed link.              |
...
| **Budget Sheet**               | Budget + Fundraising + Availability tabs, read via gviz (`src/lib/gsheets.ts`)                                                                      | Treasurer's / school account                                      | Hub money widgets show designed empties; re-point Site Settings → Budget Google Sheet ID at a copy.                                     |
...
- The /exec URL lives in `.dev.vars` (`CALENDAR_FEED_URL`) and in Sanity
  (Site Settings → Calendar feed link). Changing the URL is
