...
| **Calendar feed web app**      | Apps Script serving the calendar as JSON ([source](../scripts/apps-script/calendar-feed.gs)); skips "Tour with …" booking events (visitor/child names); carries each event's `created` date since 2026-08-29 (redeploy pending) | Maintainer's personal account (redeployed 2026-07-17)             | Redeploy from the committed source under any account that can see the calendar; update Hub settings → Calendar feed link.               |
...
- The feed serves
  `{ title, start, end?, allDay, location?, description?, created? }`; the site
  renders time ranges and dialog details from the richer fields.
- `created` (added to the source 2026-08-29) is when the event was PUT ON the
  calendar, not when it happens. The hub's what's-new bell announces an event
  added in the last 14 days. It is OPTIONAL in the site's parser
  (`src/lib/hub-calendar.ts`): the DEPLOYED script does not send it until it is
  redeployed, and until then the bell announces no Google-calendar events
  rather than failing. **Redeploy is a human step — see
  [PENDING.md](PENDING.md), "Re-paste the deployed calendar-feed script".**
