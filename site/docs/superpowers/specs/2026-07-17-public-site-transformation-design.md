...
- **Home gets the living-photograph hero.** The ambient film moves out of the full-bleed background into a taped polaroid frame beside the headline. Band surface REVISED 2026-07-17 (Nathan: "all heroes ... deep navy bg and the low opacity svgs"): the deep navy doodle drench (.wcp-hero-drench), not the original cream paper — white Captain Comic headline with the crayon underline, taped "Now enrolling" chip, tour-first CTAs, a caption strip on the frame. Interior no-media heroes share the same drench; photo heroes carry the doodle tile in their scrim. Rationale: the ~560px render downsamples the encode 2-3x so compression artifacts disappear, no scrim sits over the film so it plays at full brightness, the first fold gains the asymmetry the audit asked for, and the reduced-motion/data-saver fallback is a visually identical still in the same frame. A 720p re-encode (roughly half the bytes) is a follow-up nice-to-have.
